...
// This version of the model has more vague priors. 
...
parameters {
  ...
  real b_body_brain; // slope (effect) from body on brain, real number
  real b_body_z; // slope from body on z, real number
  real b_brain_z; // slope from brain on z, real number
...
model {
  ...
  a_brain ~ normal(0, 10);
  a_z ~ normal(0, 10);
  b_body_brain ~ normal(0, 10);
  b_body_z ~ normal(0, 10);
  b_brain_z ~ normal(0, 10);
  sigma_body ~ exponential(0.1);
  sigma_brain ~ exponential(0.1);
