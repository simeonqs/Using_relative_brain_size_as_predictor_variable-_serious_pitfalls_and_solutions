...
```{r include=FALSE}
# Libraries
## cmdstanr can be installed from Cran, but you need to run 
## library(cmstanr) and install_cmdstan() to finish installing 
library(cmdstanr)
library(scales)

# Paths
path_response_stan = 'response.stan'
path_predictor_stan = 'predictor.stan'
path_sem_stan = 'sem.stan'
path_pdf_response = 'response.pdf'
path_pdf_predictor = 'predictor.pdf'
```
...
```{r plot response, echo=FALSE, out.width = "50%", fig.cap= "Parameter estimates from the linear model and Bayesian linear model with brain size as response variable. Dashed grey line is the true value. Orange density plots are normal distributions based on the mean and SE from the linear model. Purple and green density plots are the posterior distributions for the Bayesian models."}
